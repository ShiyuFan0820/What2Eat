...
const I18N = {
  en: {
    ...
    locDeniedMacSafari: "Location was denied 🙈 Easiest: just type an address below 👇 To enable it, check BOTH places: ① Safari menu → Settings → Websites → Location → set this site to Allow; ② Apple menu → System Settings → Privacy & Security → Location Services → turn it on and tick Safari. Then quit Safari completely (⌘Q) and reopen — a refresh alone isn't enough.",
  ...
  zh: {
    ...
    locDeniedMacSafari: "定位被拒绝了 🙈 最省事：直接在下面输入地址 👇 想开启定位要检查两处：① Safari 菜单 → 设置 → 网站 → 位置 → 本站设为「允许」；② 苹果菜单 → 系统设置 → 隐私与安全性 → 定位服务 → 打开总开关并勾选 Safari。然后完全退出 Safari（⌘Q）再重新打开——只刷新页面是不够的。",
